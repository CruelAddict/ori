...
import { useConfigurationList } from "@src/ui/features/useConfigurationList";

export function ConfigurationListScreen() {
    const vm = useConfigurationList();
        ...
        <KeyScope id="configuration-list" bindings={bindings}>
            <box flexDirection="column" flexGrow={1} padding={1}>
                ...
                <Show when={!vm.loading()} fallback={<text>Loading configurations...</text>}>
                    <Show
                        when={!vm.error()}
                        fallback={<text fg="red">Failed to load configurations: {vm.error()}</text>}
                    >
                        <ConfigurationList
