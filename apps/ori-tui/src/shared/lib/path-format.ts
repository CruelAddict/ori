...
export function formatFilePath(filePath: string, maxLength = DEFAULT_MAX_LENGTH): FormattedFilePath {
  ...
  let dirPathStr = `${absolutePrefix}${dirPath.join(path.sep)}${path.sep}`
  ...
  let beforeTrunc = dirPath.slice(0, -2);
  let afterTrunc = dirPath.slice(-1);
  let dirPathTruncated = toPathStr(beforeTrunc, afterTrunc, absolutePrefix)
  while (
    dirPathTruncated.length + fileName.length > maxLength
    && (beforeTrunc.length > 0 || afterTrunc.length > 0)
  ) {
